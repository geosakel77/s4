...
class Coordinator:
    ...
    def update_agents(self,agent_uuid,response):
        for agent_uuid,agent_url in self.registered_agents.items():
            response =  self.client.check_health(agent_url)
            print(response)
        #TODO Check health before update


        #for agent_id in self.registered_agents.keys():
        #    conn_info={"DM":self.connection_data_dm,"TA":self.connection_data_ta,"IS":self.connection_data_is,"CTI":self.connection_data_cti}
