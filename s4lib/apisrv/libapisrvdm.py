from s4lib.libdm import PreventionDM,DetectionDM,ResponseDM,DM
...
class APIDMServer(APIBaseServer):
    ...
    def __init__(self,agent_type="DM",title="") -> None:
        ...
        self.agent = DM(dm_agent_uuid=self.agent_uuid,dm_agent_type=self.agent_type,dm_config=self.config,dm_type=None)
    ...
    def _register_dm_routes(self) -> None:
        ...
        async def receives_cti_product(req: Request) -> Dict[str, Any]:
            ...
            for key,value in update_data.items():
                response = self.agent.handle_indicator_from_agcti(value)
            print(response)
            return response
